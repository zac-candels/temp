...
class PressureOutflow2 : public BoundaryBase {
   public:
...
template <class TTraits, class TDistributionType>
inline void PressureOutflow2::compute(TDistributionType& distribution, int k) {
    ...
    for (int iter = 0; iter < mNumIterations; iter++) {
        ...
        for (int idx = 0; idx < TTraits::Stencil::Q; idx++) {
            if (TTraits::Stencil::Ci_x[idx] * normal[0] +
                ...
                distribution.getDistributionPointer(distribution.streamIndex(k, normalq))[idx] =
                    distribution.getEquilibriumPointer(distribution.streamIndex(k, normalq))[idx] - pressure0 * gamma/TTraits::Stencil::Cs2;
                prefactorsum += gamma;
        ...
        auto pressureModel = static_cast<ModelBase<Lattice, TTraits>*>(mModel);
    ...
    for (int idx = 1; idx < TTraits::Stencil::Q; idx++) {
        if (TTraits::Stencil::Ci_x[idx] * normal[0] +
            ...
            distribution.getDistributionPointer(distribution.streamIndex(k, normalq))[idx] =
                distribution.getEquilibriumPointer(distribution.streamIndex(k, normalq))[idx] - pressure0*gamma /TTraits::Stencil::Cs2 +
                pressure*gamma /TTraits::Stencil::Cs2;
...
template <class TTraits, class TDistributionType>
inline void PressureOutflow2::communicate(TDistributionType& distribution) {
    using Lattice = typename TTraits::Lattice;
