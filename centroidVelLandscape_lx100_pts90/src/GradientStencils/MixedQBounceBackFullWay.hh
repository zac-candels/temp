...
struct MixedQBounceBackFullWay : GradientBase<GradientMixed, AllDirections> {
    template <class TTraits, class TParameter>
    inline double compute(const int direction, const int k);
};

template <class TTraits, class TParameter>
inline double MixedQBounceBackFullWay::compute(const int direction, const int k) {
    ...
    if ((!this->isBoundary<Lattice>(data.getNeighbors()[k * Stencil::Q + direction])) &&
        ...
        return 0.25 * (-3 * param[k] + 4 * param[data.getNeighbors()[k * Stencil::Q + Stencil::Opposites[direction]]]- param[data.getNeighbors()[data.getNeighbors()[k * Stencil::Q + Stencil::Opposites[direction]] * Stencil::Q + Stencil::Opposites[direction]]]);

    } else if ((this->isBoundary<Lattice>(
                   data.getNeighbors()[data.getNeighbors()[k * Stencil::Q + direction] * Stencil::Q + direction]))) {
        if ((this->isBoundary<Lattice>(data.getNeighbors()[k * Stencil::Q + Stencil::Opposites[direction]]))) {
            return 0.25 * (
                4 * param[data.getNeighbors()[k * Stencil::Q + direction]] - 4 * param[k]);
        }
