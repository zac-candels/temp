    ...
    for (int idx = 1; idx < TTraits::Stencil::Q; idx++) {
        ...
        if (dist <= 0.5)
            distribution.getDistributionPointer(distribution.streamIndex(k, idx))[idx] =
                ...
                (dist) * (1 - 2 * dist) *
                    distribution.getPostCollisionDistribution(
                        distribution.streamIndex(distribution.streamIndex(distribution.streamIndex(k, idx), idx), idx),
                        distribution.getOpposite(idx));
        else
            distribution.getDistributionPointer(distribution.streamIndex(k, idx))[idx] =
                1.0 / (dist * (2 * dist + 1)) *
                ...
                (1 - 2 * dist) / (1 + 2 * dist) *
                    distribution.getPostCollisionDistribution(
                        distribution.streamIndex(distribution.streamIndex(k, idx), idx), idx);
    }
}
